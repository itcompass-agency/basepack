export COMPOSE_PROJECT_NAME=basepack
...
npm-install: ## Install NPM dependencies
	@make exec-by-root cmd="rm -rf node_modules"
	@make exec-by-root cmd="rm -rf package-lock.json"
...
cache-clear: ## Clears the cache
	@make exec cmd="php artisan route:cache"
...
logs-postgres: ## Shows logs from the postgres container. Use ctrl+c in order to exit
ifeq ($(INSIDE_DOCKER_CONTAINER), 0)
	@docker logs -f ${COMPOSE_PROJECT_NAME}-postgres
else
	$(ERROR_ONLY_FOR_HOST)
endif
...
migrate-force: ## Runs all migrations
	@make exec cmd="php artisan migrate --force"
